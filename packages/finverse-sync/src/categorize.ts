...
/**
 * Mirrors apps/web/src/lib/categorization.ts's rule pass (and its verbatim
 * duplicate in packages/plaid-sync/src/categorize.ts). Duplicated, not
 * imported -- apps/web isn't a workspace dependency other packages can
 * pull from; this package is the shared surface between web and worker
 * instead. Keep all three in sync if the matching logic ever changes.
 */
