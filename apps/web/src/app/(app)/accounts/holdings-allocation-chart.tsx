...
export function HoldingsAllocationChart({ data, currency }: { data: SpendByCategoryBucket[]; currency: string }) {
  return (
    <ChartContainer config={chartConfig} className="aspect-auto w-full" style={{ height: data.length * 32 + 16 }}>
      <BarChart data={data} layout="vertical" margin={{ top: 0, right: 16, bottom: 0, left: 0 }}>
        <XAxis type="number" hide />
        <YAxis type="category" dataKey="categoryName" tickLine={false} axisLine={false} width={64} fontSize={12} />
        <ChartTooltip
